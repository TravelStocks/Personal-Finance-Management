...
var palette = ["#1f5fbf", "#07835f", "#b7791f", "#6852bd", "#c53030", "#2b6cb0", "#0f766e", "#805ad5"];
var today = /* @__PURE__ */ new Date("2026-06-14T00:00:00+08:00");
var initialAccounts = [
  ...
  { id: "yu-ebao-8514", name: "\u4F59\u989D\u5B9D - 8514", type: "\u652F\u4ED8\u5B9D", balance: 5e3, purpose: "\u53BBA\u80A1", liquid: true },
...
var initialMonthRecords = [
  ...
  { id: "2026-07", label: "2026\u5E747\u6708", salary: 15e3, payday: 10, stockIncome: 0, otherIncome: 0, budgets: monthBudgets() },
  { id: "2026-08", label: "2026\u5E748\u6708", salary: 15e3, payday: 10, stockIncome: 0, otherIncome: 0, budgets: monthBudgets() },
  { id: "2026-09", label: "2026\u5E749\u6708", salary: 15e3, payday: 10, stockIncome: 0, otherIncome: 0, budgets: monthBudgets() },
  { id: "2026-10", label: "2026\u5E7410\u6708", salary: 15e3, payday: 10, stockIncome: 0, otherIncome: 0, budgets: monthBudgets() },
  { id: "2026-11", label: "2026\u5E7411\u6708", salary: 15e3, payday: 10, stockIncome: 0, otherIncome: 0, budgets: monthBudgets() },
  { id: "2026-12", label: "2026\u5E7412\u6708", salary: 15e3, payday: 10, stockIncome: 0, otherIncome: 0, budgets: monthBudgets() }
];
var initialHoldings = [
...
var reportStartMonthId = "2026-06";
var financeStorageKey = "personal-finance-management-data-v2";
var moduleList = [
...
function addMonthsToId(monthId, offset) {
...
function recordSpendingActual(record) {
  return record.budgets.reduce((sum, item) => sum + item.actual, 0);
}
function recordSpendingPlan(record) {
  return record.budgets.reduce((sum, item) => sum + item.plan, 0);
}
function investmentReserveKind(account) {
  ...
  if (text.includes("\u4E2D\u56FD\u94F6\u884C") && text.includes("8292") || text.includes("\u7F8E\u80A1") || text.includes("us stock") || text.includes("us-stock"))
    return "usShare";
  return null;
}
function isInvestmentReserveAccount(account) {
  return investmentReserveKind(account) !== null;
}
function isTravelSavingsAccount(account) {
  const text = `${account.name} ${account.type} ${account.purpose}`.toLowerCase();
  return !isInvestmentReserveAccount(account) && (text.includes("\u65C5\u6E38") || text.includes("\u65C5\u884C"));
}
function isLearningSavingsAccount(account) {
  const text = `${account.name} ${account.type} ${account.purpose}`.toLowerCase();
...
function legacyLiabilitiesFromSaved(saved) {
  return initialLiabilities.map((item) => ({
    ...item,
    amount: item.id === "house-debt" ? saved.houseDebt ?? 0 : item.id === "car-debt" ? saved.carDebt ?? 0 : saved.otherDebt ?? 0
  }));
}
function dayDistance(date) {
  const target = /* @__PURE__ */ new Date(`${date}T00:00:00+08:00`);
...
function FinanceDashboard() {
  const [activeModules, setActiveModules] = (0, import_react.useState)([]);
  const [period, setPeriod] = (0, import_react.useState)("\u6708");
  const [selectedMonth, setSelectedMonth] = (0, import_react.useState)("2026-06");
  const [monthlyRecords, setMonthlyRecords] = (0, import_react.useState)(initialMonthRecords);
  const [accounts, setAccounts] = (0, import_react.useState)(initialAccounts);
  const [holdings, setHoldings] = (0, import_react.useState)(initialHoldings);
  const [fxUsd, setFxUsd] = (0, import_react.useState)(7.25);
  const [fxHkd, setFxHkd] = (0, import_react.useState)(0.93);
  const [aSharePlan, setASharePlan] = (0, import_react.useState)(2250);
  const [usSharePlan, setUsSharePlan] = (0, import_react.useState)(1500);
  const [hkSharePlan, setHkSharePlan] = (0, import_react.useState)(0);
  const [travelSaving, setTravelSaving] = (0, import_react.useState)(3e3);
  const [learningSaving, setLearningSaving] = (0, import_react.useState)(500);
  ...
  const [futureCapabilities, setFutureCapabilities] = (0, import_react.useState)(initialFutureCapabilities);
  const [cashflowHiddenBuiltinIds, setCashflowHiddenBuiltinIds] = (0, import_react.useState)([]);
  const [cashflowCustomItems, setCashflowCustomItems] = (0, import_react.useState)([]);
  const [savedDataReady, setSavedDataReady] = (0, import_react.useState)(false);
    ...
    const loadSavedData = window.setTimeout(() => {
      try {
        ...
        if (raw) {
          ...
          } else if (typeof saved.houseDebt === "number" || typeof saved.carDebt === "number" || typeof saved.otherDebt === "number") {
  ...
  const forecast = (0, import_react.useMemo)(() => {
    ...
    const customInflow = cashflowCustomItems.filter((item) => item.direction === "inflow").reduce((sum, item) => sum + item.amount, 0);
  ...
  const cashflowEvents = (0, import_react.useMemo)(() => {
    const customInflow = cashflowCustomItems.filter((item) => item.direction === "inflow").reduce((sum, item) => sum + item.amount, 0);
  ...
  function updateMonthRecord(monthId, patch) {
    setMonthlyRecords(
      (records) => records.map((record) => record.id === monthId ? { ...record, ...patch } : record)
    );
  }
  ...
  function deleteMonthRecord(monthId) {
    setMonthlyRecords((records) => {
  ...
  function addBudget() {
    setMonthlyRecords(
      (records) => records.map(
        (record) => record.id === selectedMonth ? {
          ...record,
          budgets: [
            ...record.budgets,
            {
  ...
  function deleteBudget(id) {
    setMonthlyRecords(
      (records) => records.map(
        (record) => record.id === selectedMonth && record.budgets.length > 1 ? { ...record, budgets: record.budgets.filter((item) => item.id !== id) } : record
      )
    );
  }
  ...
  function updateFirstGoalByKind(kind, patch) {
    setGoals((items) => {
  ...
  function updateFutureCapability(id, patch) {
  ...
  const liabilitySummary = liabilities.length ? `${liabilities.slice(0, 3).map((item) => `${item.name.trim() || "\u672A\u547D\u540D\u8D1F\u503A"} ${money(item.amount)}`).join(" / ")}${liabilities.length > 3 ? " / \u66F4\u591A" : ""}` : "\u6682\u65E0\u8D1F\u503A";
  ...
  const overviewCards = [
    ...
    },
    {
      ...
      tone: totals.fixedRatio > 0.5 ? "red" : totals.fixedRatio >= 0.35 ? "amber" : "green"
    },
    {
      title: "\u672C\u6708\u5B9E\u9645\u8D44\u4EA7\u5206\u914D",
    ...
    },
    {
      ...
      tone: totals.monthlySurplus < 0 ? "red" : totals.monthlySurplus < actualIncome * 0.1 ? "amber" : "green"
    },
    {
      title: "\u5F53\u524D\u73B0\u91D1\u6D41",
  ...
  const reportMonthRecords = monthlyRecords.filter((record) => record.id >= reportStartMonthId).sort((a, b) => a.id.localeCompare(b.id));
  ...
  const cashflowRows = [
    ...
    ...cashflowCustomItems.map((item) => ({
      ...
      source: "\u81EA\u5B9A\u4E49",
      onNameChange: (value) => updateCashflowCustomItem(item.id, { name: value }),
  ...
  const assetStructureData = totalAssetBreakdown.map((item, index) => ({ item, index })).sort((left, right) => right.item.value - left.item.value || left.index - right.index).map(({ item }) => ({
    label: item.label,
    value: item.value,
    color: item.color
  }));
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", { className: "finance-page", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("aside", { className: "side-nav", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "brand", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "brand-mark", children: "PF" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "\u4E2A\u4EBA\u8D22\u52A1\u7CFB\u7EDF" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: "Sites \u7248 / \u516C\u5F00\u8131\u654F" })
        ] })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: activeModules.length === 0 ? "active" : "", onClick: () => setActiveModules([]), children: "\u603B\u89C8 / \u6E05\u7A7A" }),
      moduleList.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "button",
        {
    ...
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", { className: "workspace", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("header", { className: "topbar", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: "\u957F\u671F\u4E2A\u4EBA\u8D22\u52A1\u7BA1\u7406\u7CFB\u7EDF" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "\u6570\u636E\u5F55\u5165\u548C\u56FE\u8868\u5206\u6790\u5206\u79BB\uFF1B\u5148\u770B\u603B\u89C8\uFF0C\u518D\u591A\u9009\u6A21\u5757\u5E76\u6392\u590D\u76D8\u3002" })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "period-tabs", "aria-label": "\u65F6\u95F4\u89C6\u56FE", children: ["\u5468", "\u6708", "\u5B63", "\u5E74"].map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: period === item ? "active" : "", onClick: () => setPeriod(item), children: item }, item)) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", { className: "overview", children: [
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", { className: "analytics-section", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "section-title compact", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: "\u56FE\u8868\u5206\u6790\u533A" }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "\u8D44\u4EA7\u3001\u652F\u51FA\u3001\u73B0\u91D1\u6D41\u3001\u5065\u5EB7\u8BC4\u5206\u5206\u5F00\u770B\uFF0C\u907F\u514D\u6240\u6709\u6570\u5B57\u6324\u5728\u540C\u4E00\u5C4F\u3002" })
          ] }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid four", children: [
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u98CE\u9669\u77E9\u9635", summary: "\u5F71\u54CD \xD7 \u7D27\u8FEB", className: "wide risk-panel", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RiskMatrix, { data: riskMatrixData }) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u8D44\u91D1\u5206\u914D\u6D41\u5411", summary: `\u5206\u914D ${money(totals.assetOutflow)}`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(FlowMap, { data: outflowData, source: "\u5DE5\u8D44\u8D26\u6237", valueFormatter: money }) })
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "module-grid", children: moduleList.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
          "button",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: item.desc }),
              /* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: activeModules.includes(item.id) ? "\u5DF2\u6253\u5F00\uFF0C\u70B9\u51FB\u5173\u95ED" : "\u6253\u5F00\u8BE6\u60C5" })
            ]
          },
          item.id
        )) })
      ] }),
      activeModules.length > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", { className: "selected-modules", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "section-title selected-title", children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: "\u5DF2\u6253\u5F00\u6A21\u5757" }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "\u53EF\u4EE5\u540C\u65F6\u89C2\u770B\u591A\u4E2A\u6A21\u5757\uFF1B\u6BCF\u4E2A\u6A21\u5757\u5DE6\u4FA7\u662F\u6570\u636E\uFF0C\u53F3\u4FA7\u662F\u56FE\u8868\u3002" })
          ] }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "panel-actions", children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { onClick: openAllModules, children: "\u6253\u5F00\u5168\u90E8" }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { onClick: () => setActiveModules([]), children: "\u5168\u90E8\u6536\u8D77" })
          ] })
        ] }),
        activeModules.map((moduleId) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", { className: "detail-panel", children: [
          moduleId === "income" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u6536\u5165", desc: "\u6536\u5165\u53EA\u770B\u5B9E\u9645\u5230\u8D26\uFF1B\u7092\u80A1\u6708\u7ED3\u7B97\u5355\u72EC\u8BB0\u5F55\uFF0C\u4E0D\u8FDB\u5165\u73B0\u91D1\u6D41\u9884\u6D4B\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              ...
              charts: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid two", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u6536\u5165\u7ED3\u6784", summary: `${activeMonth.label} ${money(actualIncome)}`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DonutChart, { data: incomeChartData, centerLabel: "\u5B9E\u9645\u6536\u5165", centerValue: money(actualIncome) }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u6536\u5165\u53E3\u5F84", summary: "\u80A1\u7968\u6708\u7ED3\u4E0D\u8FDB\u9884\u6D4B", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(StackedBar, { data: incomeChartData, valueFormatter: money }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u6708\u5EA6\u6536\u5165\u8D8B\u52BF", summary: "\u6309\u6708\u4EFD\u5206\u5F00\u8BB0\u5F55", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(VerticalBarChart, { data: monthlyIncomeTrend, valueFormatter: money }) })
              ] })
            }
          ) }),
          moduleId === "spending" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u652F\u51FA", desc: "\u652F\u51FA\u4E0E\u6536\u5165\u5206\u5F00\u7BA1\u7406\uFF1B\u9884\u7B97\u3001\u5B9E\u9645\u91D1\u989D\u3001\u5FC5\u8981\u6027\u90FD\u80FD\u76F4\u63A5\u7F16\u8F91\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
          ...
          moduleId === "cashflow" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u73B0\u91D1\u6D41\u9884\u6D4B", desc: "\u672A\u6765 6 \u4E2A\u6708\u9884\u6D4B\uFF1B\u8D44\u4EA7\u5206\u914D\u4F5C\u4E3A\u73B0\u91D1\u6D41\u51FA\uFF0C\u80A1\u7968\u6536\u5165\u4E0D\u8BA1\u5165\u9884\u6D4B\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              data: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          ...
          moduleId === "accounts" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u8D26\u6237\u7BA1\u7406", desc: "\u8D26\u6237\u53EF\u4EE5\u65B0\u589E\u3001\u5220\u9664\u3001\u7F16\u8F91\u548C\u62D6\u52A8\u6392\u5E8F\uFF0C\u4FEE\u6539\u540E\u4F1A\u8054\u52A8\u603B\u8D44\u4EA7\u3001\u73B0\u91D1\u6D41\u548C\u56FE\u8868\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              data: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          ...
          moduleId === "budget" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u9884\u7B97\u7BA1\u7406", desc: "\u9884\u7B97\u6A21\u5757\u4FDD\u7559\u5468/\u6708/\u5B63/\u5E74\u89C6\u56FE\uFF0C\u56FA\u5B9A\u652F\u51FA\u7387\u9608\u503C\u4E3A 35% \u548C 50%\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              data: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MonthSelector, { records: monthlyRecords, selectedMonth, onChange: setSelectedMonth }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "stat-strip", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u5F53\u524D\u89C6\u56FE", value: `${period} / ${shortMonth(activeMonth.label)}` }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u9884\u7B97\u603B\u989D", value: money(totals.spendingPlan) }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u56FA\u5B9A\u652F\u51FA\u7387", value: percent(totals.fixedRatio) }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u9884\u7B97\u5269\u4F59", value: money(totals.spendingPlan - totals.spendingActual) })
                ] }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EditableBudgetTable, { budgets, deleteBudget, addBudget, updateBudget })
              ] }),
              charts: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid two", children: [
          ...
          moduleId === "investment" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u6295\u8D44\u7BA1\u7406", desc: "\u6295\u8D44\u7406\u8D22\u5355\u72EC\u6210\u680F\uFF0CA\u80A1 / \u7F8E\u80A1 / \u6E2F\u80A1\u5206\u5F00\u770B\uFF1B\u8BA1\u5212\u6295\u5165\u548C\u5E02\u503C\u90FD\u80FD\u6539\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              data: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "form-grid", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EditableNumber, { label: "\u7F8E\u5143\u5151\u4EBA\u6C11\u5E01", value: fxUsd, onChange: setFxUsd, step: 0.01 }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EditableNumber, { label: "\u6E2F\u5E01\u5151\u4EBA\u6C11\u5E01", value: fxHkd, onChange: setFxHkd, step: 0.01 }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EditableNumber, { label: "A\u80A1\u6708\u8BA1\u5212\u6295\u5165", value: aSharePlan, onChange: setASharePlan }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EditableNumber, { label: "\u7F8E\u80A1\u6708\u8BA1\u5212\u6295\u5165", value: usSharePlan, onChange: setUsSharePlan }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EditableNumber, { label: "\u6E2F\u80A1\u6708\u8BA1\u5212\u6295\u5165", value: hkSharePlan, onChange: setHkSharePlan })
                ] }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
              ...
              charts: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid two", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u5E02\u573A\u5206\u5E03", summary: `\u6295\u8D44\u5E02\u503C ${money(totals.investmentValue)}`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DonutChart, { data: investmentChartData, centerLabel: "\u6295\u8D44", centerValue: money(totals.investmentValue) }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u76C8\u4E8F\u7EDD\u5BF9\u503C", summary: `\u603B\u76C8\u4E8F ${money(totals.investmentPnL)}`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(HorizontalBarChart, { data: holdingRows, valueFormatter: money }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u6536\u76CA\u7387 / \u4ED3\u4F4D", summary: "\u6A2A\u8F74\u4ED3\u4F4D\uFF0C\u7EB5\u8F74\u6536\u76CA", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ScatterPlot, { data: investmentScatter, xLabel: "\u4ED3\u4F4D", yLabel: "\u6536\u76CA" }) })
              ] })
            }
          ) }),
          moduleId === "balance" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u8D44\u4EA7\u8D1F\u503A\u8868", desc: "\u8D44\u4EA7\u9879\u548C\u8D1F\u503A\u9879\u90FD\u652F\u6301\u65B0\u589E\u3001\u5220\u9664\u548C\u76F4\u63A5\u7F16\u8F91\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              data: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "stat-strip", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u603B\u8D44\u4EA7", value: money(totals.totalAssets) }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u603B\u8D1F\u503A", value: money(totals.totalDebt) }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u51C0\u8D44\u4EA7", value: money(totals.netWorth) }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u8D1F\u503A\u7387", value: percent(totals.debtRatio) })
                ] })
              ] }),
              charts: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid two", children: [
          ...
          moduleId === "emergency" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u5E94\u6025\u91D1", desc: "\u5F53\u524D\u91D1\u989D\u4E0E\u76EE\u6807\u7BA1\u7406\u7684\u5E94\u6025\u50A8\u5907\u81EA\u52A8\u540C\u6B65\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              ...
              charts: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid two", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u5E94\u6025\u91D1\u8986\u76D6", summary: `${totals.emergencyCoverage.toFixed(1)} / ${emergencyMonths} \u4E2A\u6708`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  ProgressList,
                  {
                    data: [
                      {
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u5FC5\u8981\u652F\u51FA\u538B\u529B", summary: `\u5E94\u6025\u57FA\u51C6 ${money(totals.emergencyMonthlyNeed)}`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                  HorizontalBarChart,
                  {
                    data: [
          ...
          moduleId === "reminders" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u8D26\u5355\u4E0E\u63D0\u9192", desc: "\u7F51\u9875\u5185\u63D0\u9192\uFF0C\u9ED8\u8BA4\u63D0\u524D 7 \u5929\uFF1B\u91D1\u989D\u53EF\u5148\u624B\u52A8\u7EF4\u62A4\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              data: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                EditableReminderTable,
              ...
              charts: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid two", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u63D0\u9192\u91D1\u989D", summary: "\u907F\u514D\u6F0F\u6263", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(HorizontalBarChart, { data: reminderAmountData, valueFormatter: money }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u8DDD\u79BB\u5230\u671F", summary: "\u4EE5 2026-06-14 \u4E3A\u5F53\u524D\u65E5", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(VerticalBarChart, { data: reminderDaysData, valueFormatter: (value) => `${value.toFixed(0)}\u5929` }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u63D0\u9192\u65E5\u5386", summary: "\u672A\u6765\u5173\u952E\u6263\u6B3E", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CashflowCalendar, { events: cashflowEvents }) })
              ] })
            }
          ) }),
          ...
          moduleId === "reports" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u8D22\u52A1\u62A5\u8868", desc: "\u6708\u5EA6\u590D\u76D8\u5148\u7ED9\u5173\u952E\u7ED3\u8BBA\uFF0C\u540E\u7EED\u53EF\u4EE5\u63A5\u5386\u53F2\u6570\u636E\u505A\u8D8B\u52BF\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              data: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "stat-strip", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u6536\u5165", value: money(actualIncome) }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u652F\u51FA", value: money(totals.spendingActual) }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u7ED3\u4F59", value: money(totals.monthlySurplus) }),
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { className: "review-copy", children: [
                  ...
                  money(actualIncome),
                  "\uFF0C\u5DF2\u8BB0\u5F55\u652F\u51FA ",
                  money(totals.spendingActual),
                  "\uFF0C \u8D44\u4EA7\u5206\u914D ",
                  money(totals.assetOutflow),
                  "\u3002\u6295\u8D44\u6D6E\u52A8\u76C8\u4E8F ",
                  money(totals.investmentPnL),
                  "\u3002 \u4E0B\u6708\u91CD\u70B9\u5173\u6CE8\u73B0\u91D1\u6D41\u9884\u6D4B\u3001\u56FA\u5B9A\u652F\u51FA\u7387\u548C 7 \u5929\u5185\u63D0\u9192\u3002"
                ] })
              ] }),
              charts: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid two", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u672C\u6708\u8D44\u91D1\u6D41\u5411", summary: `\u50A8\u84C4\u7387 ${percent(totals.savingsRate)}`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(VerticalBarChart, { data: reportData, valueFormatter: money }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u6708\u5EA6\u6536\u652F\u5BF9\u6BD4", summary: "\u6536\u5165\u548C\u652F\u51FA\u5206\u6708\u67E5\u770B", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(VerticalBarChart, { data: [...monthlyIncomeTrend, ...monthlySpendingTrend], valueFormatter: money }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u73B0\u91D1\u6D41\u672A\u6765\u8D8B\u52BF", summary: `6\u4E2A\u6708\u672B ${money(forecast[forecast.length - 1]?.balance ?? 0)}`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LineChart, { data: cashflowLine, valueFormatter: money }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u8D44\u91D1\u7011\u5E03\u590D\u76D8", summary: "\u6536\u5165\u3001\u652F\u51FA\u3001\u5206\u914D", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(WaterfallChart, { data: waterfallData }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u98CE\u9669\u77E9\u9635", summary: "\u4E0B\u6708\u5173\u6CE8\u70B9", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RiskMatrix, { data: riskMatrixData }) })
              ] })
            }
          ) }),
          moduleId === "health" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u8D22\u52A1\u5065\u5EB7\u8BC4\u5206", desc: "100 \u5206\u5236\uFF0C\u7528\u73B0\u91D1\u6D41\u3001\u5E94\u6025\u91D1\u3001\u8D1F\u503A\u3001\u589E\u957F\u548C\u8D8B\u52BF\u7EFC\u5408\u5224\u65AD\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              data: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "health-score", style: { "--score": `${totals.score}%` }, children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: totals.score }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u8D22\u52A1\u5065\u5EB7\u5206" })
                ] }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
                ...
                /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "stat-strip", children: [
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u56FA\u5B9A\u652F\u51FA\u7387", value: percent(totals.fixedRatio) }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u5E94\u6025\u8986\u76D6", value: `${totals.emergencyCoverage.toFixed(1)}\u4E2A\u6708` }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u8D1F\u503A\u7387", value: percent(totals.debtRatio) }),
                  /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Stat, { label: "\u6295\u8D44\u76C8\u4E8F", value: money(totals.investmentPnL) })
                ] })
              ] }),
              charts: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid two", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u5065\u5EB7\u7EF4\u5EA6\u62C6\u89E3", summary: "\u4E94\u9879\u5206\u6570", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(HorizontalBarChart, { data: healthDimensions, valueFormatter: (value) => `${value.toFixed(0)}\u5206`, percentMode: true }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u8D44\u4EA7\u6297\u98CE\u9669\u7ED3\u6784", summary: "\u8D26\u6237 / \u5E94\u6025 / \u8D1F\u503A", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(DonutChart, { data: assetStructureData, centerLabel: "\u8986\u76D6", centerValue: `${totals.emergencyCoverage.toFixed(1)}\u6708` }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u98CE\u9669\u77E9\u9635", summary: "\u5F71\u54CD \xD7 \u7D27\u8FEB", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RiskMatrix, { data: riskMatrixData }) })
              ] })
            }
          ) }),
          moduleId === "future" && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Module, { title: "\u6570\u636E\u80FD\u529B", desc: "\u503A\u52A1\u3001\u4FDD\u9669\u3001\u6570\u636E\u8D28\u91CF\u3001\u89C4\u5219\u5F15\u64CE\u5148\u653E\u7ED3\u6784\uFF0C\u7B49\u540E\u7EED\u6570\u636E\u8865\u9F50\u518D\u6FC0\u6D3B\u3002", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            DataChartLayout,
            {
              data: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(EditableCapabilityTable, { capabilities: futureCapabilities, updateCapability: updateFutureCapability }),
              charts: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-grid two", children: [
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u80FD\u529B\u6210\u719F\u5EA6", summary: "\u672A\u6765\u6A21\u5757\u5360\u4F4D\u8BC4\u5206", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(HorizontalBarChart, { data: futureCapabilityData, valueFormatter: (value) => `${value.toFixed(0)}\u5206`, percentMode: true }) }),
                /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChartPanel, { title: "\u89C4\u5219\u8986\u76D6\u8DEF\u7EBF", summary: "\u81EA\u52A8\u5206\u7C7B / \u6821\u9A8C / \u63D0\u9192", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(StackedBar, { data: futureCapabilityData, valueFormatter: (value) => `${value.toFixed(0)}\u5206` }) })
              ] })
            }
          ) })
        ] }, moduleId))
...
function toCny(value, currency, fxUsd, fxHkd) {
  if (currency === "USD") return value * fxUsd;
  if (currency === "HKD") return value * fxHkd;
  return value;
}
function fixedRatioLabel(value) {
  if (value < 0.35) return "\u4F4E\u4E8E 35%\uFF0C\u5065\u5EB7";
  if (value <= 0.5) return "35%-50%\uFF0C\u6CE8\u610F";
  return "\u9AD8\u4E8E 50%\uFF0C\u98CE\u9669";
}
function DataChartLayout({ data, charts }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "data-chart-layout", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", { className: "data-pane", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "pane-label", children: "\u6570\u636E\u533A" }),
      data
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", { className: "chart-pane", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "pane-label", children: "\u56FE\u8868\u533A" }),
      charts
    ] })
  ] });
}
function Module({ title, desc, children }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "section-title", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: title }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: desc })
    ] }) }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "module-body", children })
  ] });
}
function Stat({ label, value }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", { className: "stat-card", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: label }),
...
function MonthSelector({
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "month-selector-shell", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "month-selector", "aria-label": "\u6708\u4EFD\u5207\u6362", children: records.map((record) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
      "button",
      {
        ...
        children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: shortMonth(record.label) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: record.id === selectedMonth ? "\u5F53\u524D" : "\u5207\u6362" })
        ]
      },
      record.id
    )) }),
    (onAddMonth || onDeleteSelectedMonth) && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "month-selector-actions", children: [
      onAddMonth && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "secondary-button", type: "button", onClick: onAddMonth, children: "\u65B0\u589E\u6708\u4EFD" }),
      onDeleteSelectedMonth && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "button",
        {
...
function ChartPanel({ title, summary, children, className = "" }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", { className: `chart-panel ${className}`.trim(), children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-head", children: [
...
function TableSelect({
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("select", { className: "table-input", "aria-label": ariaLabel, value, onChange: (event) => onChange(event.target.value), children: options.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", { value: item, children: item }, item)) });
}
function TableCheckbox({ checked, onChange, ariaLabel }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
    "input",
    {
      "aria-label": ariaLabel,
...
function TableToolbar({ title, meta, action }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "table-toolbar", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: title }),
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      TableToolbar,
      {
        ...
        action: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "secondary-button", type: "button", onClick: addMonthRecord, children: "\u65B0\u589E\u6708\u4EFD" })
      }
    ),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u6708\u4EFD" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5DE5\u8D44" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5230\u8D26\u65E5" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u7092\u80A1\u6708\u7ED3" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5176\u4ED6\u6536\u5165" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5B9E\u9645\u6536\u5165" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u72B6\u6001" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u64CD\u4F5C" })
      ] }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: records.map((record) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { className: record.id === selectedMonth ? "selected-row" : "", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "row-select-button", type: "button", onClick: () => onSelect(record.id), children: record.label }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: `${record.label} \u5DE5\u8D44`, value: record.salary, onChange: (value) => updateMonthRecord(record.id, { salary: value }) }) }),
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { className: "calculated-cell", children: money(recordIncome(record)) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: record.id === selectedMonth ? "pill good" : "pill", children: record.id === selectedMonth ? "\u5F53\u524D" : "\u53EF\u9009" }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          "button",
          {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      TableToolbar,
      {
        ...
        action: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "secondary-button", type: "button", onClick: addCashflowCustomItem, children: "\u65B0\u589E\u73B0\u91D1\u6D41" })
      }
    ),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u53E3\u5F84" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u64CD\u4F5C" })
      ] }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tbody", { children: [
        rows.length === 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tr", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { className: "calculated-cell", colSpan: 4, children: "\u6682\u65E0\u73B0\u91D1\u6D41\u884C\uFF0C\u70B9\u51FB\u65B0\u589E\u73B0\u91D1\u6D41\u5F00\u59CB\u5F55\u5165\u3002" }) }),
        rows.map((row) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: row.onNameChange ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableTextInput, { ariaLabel: `${row.name} \u9879\u76EE`, value: row.name, onChange: row.onNameChange }) : row.name }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: row.readonlyAmount || !row.onAmountChange ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "calculated-cell", children: money(row.amount) }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: `${row.name} \u6570\u503C`, value: row.amount, onChange: row.onAmountChange }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: row.onDirectionChange ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            TableSelect,
            {
...
function EditableAccountsTable({
  ...
  const [draggedAccountId, setDraggedAccountId] = (0, import_react.useState)(null);
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      TableToolbar,
      {
        ...
        action: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "secondary-button", type: "button", onClick: addAccount, children: "\u65B0\u589E\u8D26\u6237" })
      }
    ),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { className: "drag-column", children: "\u6392\u5E8F" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u8D26\u6237\u540D\u79F0" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u7C7B\u578B" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u4F59\u989D" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u7528\u9014" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u53EF\u52A8\u7528" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u64CD\u4F5C" })
      ] }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: accounts.map((item, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
        "tr",
        {
          ...
          children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { className: "drag-column", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
              "span",
              {
            ...
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableTextInput, { ariaLabel: `${item.name} \u8D26\u6237\u540D\u79F0`, value: item.name, onChange: (value) => updateAccount(item.id, { name: value }) }) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableTextInput, { ariaLabel: `${item.name} \u7C7B\u578B`, value: item.type, onChange: (value) => updateAccount(item.id, { type: value }) }) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: `${item.name} \u4F59\u989D`, value: item.balance, onChange: (value) => updateAccount(item.id, { balance: value }) }) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableTextInput, { ariaLabel: `${item.name} \u7528\u9014`, value: item.purpose, onChange: (value) => updateAccount(item.id, { purpose: value }) }) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableCheckbox, { ariaLabel: `${item.name} \u53EF\u52A8\u7528`, checked: item.liquid, onChange: (value) => updateAccount(item.id, { liquid: value }) }) }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "danger-button compact", disabled: accounts.length <= 1, type: "button", onClick: () => deleteAccount(item.id), children: "\u5220\u9664" }) })
          ]
        },
        item.id
      )) })
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      TableToolbar,
      {
        ...
        action: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "secondary-button", type: "button", onClick: addHolding, children: "\u65B0\u589E\u6301\u4ED3" })
      }
    ),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u540D\u79F0" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5E02\u573A" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5E01\u79CD" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u6210\u672C" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5F53\u524D\u5E02\u503C" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u6298\u4EBA\u6C11\u5E01" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u76C8\u4E8F" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u64CD\u4F5C" })
      ] }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: holdings.map((item) => {
        const valueCny = toCny(item.value, item.currency, fxUsd, fxHkd);
        const costCny = toCny(item.cost, item.currency, fxUsd, fxHkd);
        return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableTextInput, { ariaLabel: `${item.name} \u540D\u79F0`, value: item.name, onChange: (value) => updateHolding(item.id, { name: value }) }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            TableSelect,
            {
              ...
              options: ["A\u80A1", "\u7F8E\u80A1", "\u6E2F\u80A1"],
              value: item.market,
              onChange: (value) => updateHolding(item.id, { market: value })
            }
          ) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            TableSelect,
            {
              ...
              options: ["CNY", "USD", "HKD"],
              value: item.currency,
              onChange: (value) => updateHolding(item.id, { currency: value })
            }
          ) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: `${item.name} \u6210\u672C`, value: item.cost, onChange: (value) => updateHolding(item.id, { cost: value }) }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: `${item.name} \u5F53\u524D\u5E02\u503C`, value: item.value, onChange: (value) => updateHolding(item.id, { value }) }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { className: "calculated-cell", children: money(valueCny) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { className: valueCny >= costCny ? "positive" : "negative", children: money(valueCny - costCny) }),
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      TableToolbar,
      {
        ...
        action: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "secondary-button", type: "button", onClick: addLiability, children: "\u65B0\u589E\u8D1F\u503A" })
      }
    ),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u8D1F\u503A\u9879" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u91D1\u989D" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5360\u6BD4" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u64CD\u4F5C" })
      ] }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: liabilities.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
          TableTextInput,
          {
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { className: "calculated-cell", children: percent(total ? item.amount / total : 0) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "danger-button compact", disabled: liabilities.length <= 1, type: "button", onClick: () => deleteLiability(item.id), children: "\u5220\u9664" }) })
      ] }, item.id)) })
    ] }) })
  ] });
}
function EditableEmergencyTable({
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableToolbar, { title: "\u5E94\u6025\u91D1\u5E95\u8868", meta: `\u76EE\u6807 ${money(target)} / \u5F53\u524D ${money(emergencyFund)}` }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u9879\u76EE" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u6570\u503C" }),
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tbody", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: "\u5F53\u524D\u5E94\u6025\u91D1" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: "\u5F53\u524D\u5E94\u6025\u91D1", value: emergencyFund, onChange: setEmergencyFund }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("td", { className: "calculated-cell", children: [
            "\u8986\u76D6 ",
            emergencyMonthlyNeed ? (emergencyFund / emergencyMonthlyNeed).toFixed(1) : "0.0",
            " \u6708"
          ] })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: "\u76EE\u6807\u8986\u76D6\u6708\u6570" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: "\u76EE\u6807\u8986\u76D6\u6708\u6570", step: 1, value: emergencyMonths, onChange: setEmergencyMonths }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("td", { className: "calculated-cell", children: [
            emergencyMonths,
            " \u4E2A\u6708"
          ] })
        ...
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: "\u6708\u5747\u5FC5\u8981\u652F\u51FA" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: "\u6708\u5747\u5FC5\u8981\u652F\u51FA", value: emergencyMonthlyNeed, onChange: setEmergencyMonthlyNeed }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("td", { className: "calculated-cell", children: [
            "\u7F3A\u53E3 ",
            money(Math.max(0, target - emergencyFund))
          ] })
        ] })
      ] })
    ] }) })
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      TableToolbar,
      {
        ...
        action: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "secondary-button", type: "button", onClick: addReminder, children: "\u65B0\u589E\u63D0\u9192" })
      }
    ),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u4E8B\u9879" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u65E5\u671F" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u91D1\u989D" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u7C7B\u578B" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u8DDD\u79BB" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u64CD\u4F5C" })
      ] }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: reminders.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableTextInput, { ariaLabel: `${item.name} \u4E8B\u9879`, value: item.name, onChange: (value) => updateReminder(item.id, { name: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableDateInput, { ariaLabel: `${item.name} \u65E5\u671F`, value: item.date, onChange: (value) => updateReminder(item.id, { date: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: `${item.name} \u91D1\u989D`, value: item.amount, onChange: (value) => updateReminder(item.id, { amount: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableTextInput, { ariaLabel: `${item.name} \u7C7B\u578B`, value: item.kind, onChange: (value) => updateReminder(item.id, { kind: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("td", { className: "calculated-cell", children: [
          Math.max(0, dayDistance(item.date)),
          " \u5929"
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "danger-button compact", disabled: reminders.length <= 1, type: "button", onClick: () => deleteReminder(item.id), children: "\u5220\u9664" }) })
      ] }, item.id)) })
    ] }) })
  ] });
}
function EditableGoalsTable({
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: goals.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    ...
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u9879\u76EE" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u6570\u503C" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5F71\u54CD" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u64CD\u4F5C" })
      ] }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tbody", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: "\u76EE\u6807\u8986\u76D6\u6708\u6570" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: "\u5065\u5EB7\u76EE\u6807\u8986\u76D6\u6708\u6570", step: 1, value: emergencyMonths, onChange: setEmergencyMonths }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: "\u6297\u98CE\u9669\u80FD\u529B" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { className: "calculated-cell", children: "\u56FA\u5B9A\u9879" })
        ] }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: "\u6708\u5747\u5FC5\u8981\u652F\u51FA" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: "\u5065\u5EB7\u6708\u5747\u5FC5\u8981\u652F\u51FA", value: emergencyMonthlyNeed, onChange: setEmergencyMonthlyNeed }) }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: "\u5E94\u6025\u8986\u76D6" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { className: "calculated-cell", children: "\u56FA\u5B9A\u9879" })
        ] }),
        liabilities.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
            TableTextInput,
            {
          ...
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: "\u8D1F\u503A\u7387" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "danger-button compact", disabled: liabilities.length <= 1, type: "button", onClick: () => deleteLiability(item.id), children: "\u5220\u9664" }) })
        ] }, item.id))
      ] })
    ] }) })
...
function EditableCapabilityTable({
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableToolbar, { title: "\u6570\u636E\u80FD\u529B\u5E95\u8868", meta: `\u5E73\u5747\u6210\u719F\u5EA6 ${average.toFixed(0)} \u5206` }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u80FD\u529B" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u6210\u719F\u5EA6" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u72B6\u6001" })
      ] }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: capabilities.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableTextInput, { ariaLabel: `${item.name} \u540D\u79F0`, value: item.name, onChange: (value) => updateCapability(item.id, { name: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: `${item.name} \u6210\u719F\u5EA6`, max: 100, step: 1, value: item.score, onChange: (value) => updateCapability(item.id, { score: clamp(value) }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: item.score >= 70 ? "pill good" : item.score >= 40 ? "pill warn" : "pill", children: item.score >= 70 ? "\u53EF\u7528" : item.score >= 40 ? "\u5EFA\u8BBE\u4E2D" : "\u5F85\u8865\u9F50" }) })
      ] }, item.id)) })
    ] }) })
  ] });
}
...
function HorizontalBarChart({
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "horizontal-bars", children: data.map((item, index) => {
    const denominator = item.max ?? max;
    const width = clamp(item.value / Math.max(denominator, 1) * 100);
    return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "bar-row", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "bar-meta", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: item.label }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: item.detail ?? valueFormatter(item.value) })
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "bar-track", "aria-label": `${item.label} ${valueFormatter(item.value)}`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "span",
        {
          className: "bar-fill",
...
function VerticalBarChart({ data, valueFormatter }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "vertical-bars", children: data.map((item, index) => {
    const height = clamp(item.value / max * 100, 4, 100);
    return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "vbar", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "vbar-frame", "aria-label": `${item.label} ${valueFormatter(item.value)}`, children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "span",
        {
          className: "vbar-fill",
...
function LineChart({ data, valueFormatter }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "line-chart-wrap", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", { className: "line-chart", viewBox: `0 0 ${width} ${height}`, role: "img", "aria-label": "\u8D8B\u52BF\u6298\u7EBF\u56FE", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("path", { d: areaPath, fill: "rgba(31, 95, 191, 0.12)" }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("polyline", { fill: "none", points: pointString, stroke: "#1f5fbf", strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: "3" }),
      points.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("g", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("circle", { cx: item.x, cy: item.y, fill: "#ffffff", r: "4", stroke: "#1f5fbf", strokeWidth: "2" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("text", { className: "line-label", x: item.x, y: height - 5, children: item.label })
      ] }, item.label))
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-caption", children: [
...
function StackedBar({ data, valueFormatter }) {
  const rows = data.filter((item) => item.value > 0);
  const total = rows.reduce((sum, item) => sum + item.value, 0);
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "stacked-wrap", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "stacked-bar", "aria-label": `\u5408\u8BA1 ${valueFormatter(total)}`, children: rows.length ? rows.map((item, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      "span",
      {
        style: {
      ...
      item.label
    )) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { width: "100%", backgroundColor: "#d8e2ee" } }) }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Legend, { data: rows.length ? rows : [{ label: "\u6682\u65E0\u6570\u636E", value: 0, color: "#b8c4d4" }], valueFormatter })
  ] });
}
function ProgressList({ data, valueFormatter }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "progress-list", children: data.map((item, index) => {
    const width = clamp(item.value / Math.max(item.max ?? item.value, 1) * 100);
    return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "progress-row", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "bar-meta", children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: item.label }),
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "progress-track", children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "span",
        {
      ...
      item.detail && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: item.detail })
    ] }, item.label);
  }) });
}
function CashflowCalendar({ events }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "cashflow-calendar", children: events.map((event) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("article", { className: "calendar-event", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: event.date.slice(5) }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: event.item }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
      event.inflow > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("em", { className: "positive", children: [
        "+",
        money(event.inflow)
      ] }),
      event.outflow > 0 && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("em", { className: "negative", children: [
        "-",
        money(event.outflow)
      ] })
    ] }),
...
function WaterfallChart({ data }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "waterfall-wrap", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", { className: "waterfall-chart", viewBox: `0 0 ${width} ${height}`, role: "img", "aria-label": "\u73B0\u91D1\u7011\u5E03\u56FE", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("line", { className: "axis-line", x1: pad, x2: width - pad, y1: scaleY(0), y2: scaleY(0) }),
      bars.map((item, index) => {
        ...
        return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("g", { children: [
          ...
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("text", { className: "waterfall-label", x: x + barWidth / 2, y: height - 6, children: item.label })
        ] }, item.label);
      })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "chart-caption", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
        "\u6D41\u5165 ",
        money(data.filter((item) => item.value > 0 && item.kind !== "start" && item.kind !== "end").reduce((sum, item) => sum + item.value, 0))
      ] }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", { children: [
        "\u6D41\u51FA ",
        money(Math.abs(data.filter((item) => item.value < 0).reduce((sum, item) => sum + item.value, 0)))
      ] })
    ] })
  ] });
}
function RiskMatrix({ data }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "matrix-layout", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", { className: "risk-matrix", viewBox: `0 0 ${size} ${size}`, role: "img", "aria-label": "\u98CE\u9669\u77E9\u9635", children: [
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("rect", { className: "matrix-zone mid", height: (size - pad * 2) / 2, width: (size - pad * 2) / 2, x: pad + (size - pad * 2) / 2, y: pad + (size - pad * 2) / 2 }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("rect", { className: "matrix-zone mid", height: (size - pad * 2) / 2, width: (size - pad * 2) / 2, x: pad, y: pad }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("rect", { className: "matrix-zone high", height: (size - pad * 2) / 2, width: (size - pad * 2) / 2, x: pad + (size - pad * 2) / 2, y: pad }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("line", { className: "axis-line", x1: pad, x2: size - pad, y1: size - pad, y2: size - pad }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("line", { className: "axis-line", x1: pad, x2: pad, y1: pad, y2: size - pad }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("text", { className: "matrix-axis", x: size / 2, y: size - 5, children: "\u5F71\u54CD" }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("text", { className: "matrix-axis vertical", x: "12", y: size / 2, children: "\u7D27\u8FEB" }),
      data.map((item) => {
        const x = pad + clamp(item.x) / 100 * (size - pad * 2);
        const y = size - pad - clamp(item.y) / 100 * (size - pad * 2);
        return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("g", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("circle", { cx: x, cy: y, fill: item.color ?? palette[0], r: "8" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("text", { className: "matrix-point-label", x, y: y - 13, children: item.label })
        ] }, item.label);
      })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Legend, { data: data.map((item) => ({ label: item.label, value: 0, detail: item.value, color: item.color })), valueFormatter: () => "" })
  ] });
}
function FlowMap({ data, source, valueFormatter }) {
  const max = Math.max(...data.map((item) => item.value), 1);
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flow-map", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flow-source", children: source }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "flow-lines", children: data.map((item, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "flow-row", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        "span",
        {
          style: {
      ...
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: item.label }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: valueFormatter(item.value) })
    ] }, item.label)) })
  ] });
}
function HeatmapGrid({ data }) {
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "heatmap-grid", children: data.map((item, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
    "div",
    {
      className: "heat-cell",
      style: {
      ...
      children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: item.label }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: item.detail ?? `${item.value.toFixed(0)}%` })
      ]
    },
    item.label
  )) });
...
function ScatterPlot({ data, xLabel, yLabel }) {
  ...
  const pad = 28;
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "matrix-layout", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("svg", { className: "risk-matrix", viewBox: `0 0 ${size} ${size}`, role: "img", "aria-label": `${xLabel} \u548C ${yLabel} \u6563\u70B9\u56FE`, children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("rect", { className: "matrix-zone low", height: size - pad * 2, width: size - pad * 2, x: pad, y: pad }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("line", { className: "axis-line", x1: pad, x2: size - pad, y1: size - pad, y2: size - pad }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("line", { className: "axis-line", x1: pad, x2: pad, y1: pad, y2: size - pad }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("text", { className: "matrix-axis", x: size / 2, y: size - 5, children: xLabel }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("text", { className: "matrix-axis vertical", x: "12", y: size / 2, children: yLabel }),
      data.map((item) => {
        const x = pad + clamp(item.x) / 100 * (size - pad * 2);
        const y = size - pad - clamp(item.y) / 100 * (size - pad * 2);
        return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("g", { children: [
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("circle", { cx: x, cy: y, fill: item.color ?? palette[0], r: "7" }),
          /* @__PURE__ */ (0, import_jsx_runtime.jsx)("text", { className: "matrix-point-label", x, y: y - 10, children: item.label })
        ] }, item.label);
      })
    ] }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Legend, { data: data.map((item) => ({ label: item.label, value: 0, detail: item.value, color: item.color })), valueFormatter: () => "" })
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
      TableToolbar,
      {
        ...
        action: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "secondary-button", type: "button", onClick: addBudget, children: "\u65B0\u589E\u5206\u7C7B" })
      }
    ),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "table-wrap spreadsheet-wrap", children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("table", { className: "spreadsheet-table", children: [
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("thead", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5206\u7C7B" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u9884\u7B97" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5B9E\u9645" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5269\u4F59" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u5FC5\u987B" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u56FA\u5B9A" }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("th", { children: "\u64CD\u4F5C" })
      ] }) }),
      /* @__PURE__ */ (0, import_jsx_runtime.jsx)("tbody", { children: budgets.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("tr", { children: [
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableTextInput, { ariaLabel: `${item.name} \u5206\u7C7B`, value: item.name, onChange: (value) => updateBudget(item.id, { name: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: `${item.name} \u9884\u7B97`, value: item.plan, onChange: (value) => updateBudget(item.id, { plan: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableNumberInput, { ariaLabel: `${item.name} \u5B9E\u9645`, value: item.actual, onChange: (value) => updateBudget(item.id, { actual: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { className: "calculated-cell", children: money(item.plan - item.actual) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableCheckbox, { ariaLabel: `${item.name} \u5FC5\u987B`, checked: item.required, onChange: (value) => updateBudget(item.id, { required: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TableCheckbox, { ariaLabel: `${item.name} \u56FA\u5B9A`, checked: item.fixed, onChange: (value) => updateBudget(item.id, { fixed: value }) }) }),
        /* @__PURE__ */ (0, import_jsx_runtime.jsx)("td", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { className: "danger-button compact", disabled: budgets.length <= 1, type: "button", onClick: () => deleteBudget(item.id), children: "\u5220\u9664" }) })
      ] }, item.id)) })
    ] }) })
  ] });
}
